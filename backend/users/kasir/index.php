...
// a. Jumlah pesanan online yang perlu divalidasi
$sql_validasi = "SELECT COUNT(id_pesanan) as total FROM pesanan WHERE status_pesanan = 'menunggu_konfirmasi'";
$result_validasi = mysqli_query($koneksi, $sql_validasi);
$jumlah_validasi = mysqli_fetch_assoc($result_validasi)['total'] ?? 0;

// b. Jumlah pesanan yang sedang diproses
...
// c. Jumlah pesanan yang masih pending (antre)
...
// d. Jumlah transaksi yang dilayani oleh kasir ini hari ini
$today = date('Y-m-d');
$stmt_pribadi = mysqli_prepare($koneksi, "SELECT COUNT(id_pesanan) as total FROM pesanan WHERE id_karyawan = ? AND DATE(tgl_pesanan) = ?");
                        ...
                        <div class="col-lg-6">
                            <div class="card card-link bg-info text-white mb-4">
                                <div class="card-body py-4 text-center">
                                    ...
                                    <p class="card-text">Validasi pesanan online & kelola antrean dapur.</p>
                                </div>
                            ...
                            <div class="card bg-danger text-white mb-4">
                                <div class="card-body">
                                    <div class="d-flex justify-content-between">
                                        <div class="fs-1 fw-bold"><?php echo $jumlah_validasi; ?></div>
                                        <div><i class="fas fa-money-check-alt fa-3x opacity-50"></i></div>
                                    </div>
                                    <p class="mb-0">Pesanan Perlu Validasi</p>
                                </div>
                            </div>
                        </div>
                         <div class="col-xl-4 col-md-6">
